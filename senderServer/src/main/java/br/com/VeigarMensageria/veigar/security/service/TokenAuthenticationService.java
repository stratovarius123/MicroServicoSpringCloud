package br.com.VeigarMensageria.veigar.security.service;
...
public class TokenAuthenticationService {
    ...
    public static void addAuthentication(HttpServletResponse response, String username) throws UnsupportedEncodingException {
        String JWT = Jwts.builder()
                ...
                .signWith(SignatureAlgorithm.HS512, SECRET.getBytes(UTF8))
                .compact();
    ...
    public static Authentication getAuthentication(HttpServletRequest request) {
    	...
    	try{
        if (token != null) {
            String user = Jwts.parser().setSigningKey(SECRET.getBytes(UTF8)).parseClaimsJws(token.replace(TOKEN_PREFIX, "")).getBody().getSubject();
            if (user != null) {
